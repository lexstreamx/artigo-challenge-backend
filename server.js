...
if (!FRONTEND_URL || !LEARNWORLDS_SCHOOL_URL) {
    console.error("Critical environment variables are missing! Ensure FRONTEND_URL and LEARNWORLDS_SCHOOL_URL are set.");
    process.exit(1);
}
...
// --- DATABASE OF QUESTIONS (Truncated for brevity, assuming full list is present) ---
const allQuestions = [
    // --- Processo Civil ---
    ...
    // ... (All other 89 questions go here) ...
...
const checkAuth = async (req, res, next) => {
    ...
    try {
        ...
        if (error.response) {
            ...
            // Send the same status code Learnworlds sent us.
...
app.listen(PORT, () => {
    ...
    console.log(`Frontend URL configured for CORS: ${FRONTEND_URL}`);
    console.log(`Learnworlds School URL for API calls: ${LEARNWORLDS_SCHOOL_URL}`);
});
